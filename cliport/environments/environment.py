...
from cliport.utils.const import colors
...
class Environment(gym.Env):
    ...
    def change_visual(self, plane, table):
        p.changeVisualShape(plane, -1, rgbaColor=[1, 0, 0, 1])
        p.changeVisualShape(table, -1, rgbaColor=[1, 1, 0, 1])

    def reset(self, augment=False):
        ...
        if augment:
            ...
            #color_random.seed(42)
            random_index = color_random.randint(0, len(colors) - 1)
            random_color_plane = colors[random_index]
            remaining_colors = [color for idx, color in enumerate(colors) if idx != random_index] # list except random_index
            random_color_table = color_random.choice(remaining_colors)
            print(f'"Plane": {random_color_plane}, "Table": {random_color_table}')

            p.changeVisualShape(plane, -1, rgbaColor=random_color_plane)
            p.changeVisualShape(table, -1, rgbaColor=random_color_table)
    ...
    def movep(self, pose, speed=0.01):
        ...
        # print(p.getBasePositionAndOrientation(self.ur5))
