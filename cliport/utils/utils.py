...
def sample_center_distribution(prob, n_samples=1):
    """Sample data point from a custom distribution."""
    labeled_mask, num_features = scipy.ndimage.label(prob)
    object_centers = np.array(scipy.ndimage.center_of_mass(prob, labeled_mask, range(1, num_features + 1))) # calculate center of mass of each objects
    random_center = object_centers[np.random.choice(len(object_centers))]
    return np.int32(random_center)

def get_nearest_object_id(pose, threshold=0.1):
    """
    Find nearest object id depending on position
    """
    nearest_object_id = None
    nearest_distance = float('inf')
    
    num_bodies = p.getNumBodies()
    for obj_id in range(num_bodies):
        obj_pos, _ = p.getBasePositionAndOrientation(obj_id)
        
        distance = np.linalg.norm(np.array(pose) - np.array(obj_pos))
...
def get_object_length_along_axis(object_id, theta): # rotated angle of z : theta
    aabb_min, aabb_max = p.getAABB(object_id) # bbox min max xyz coord
    corners = np.array([
    ...
    projections = np.dot(corners, direction_vector)
    object_length = projections.max() - projections.min()
