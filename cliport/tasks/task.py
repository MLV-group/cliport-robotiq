...
from cliport.tasks.grippers import Suction, FingeredGripper
...
class Task():
    ...
    def oracle(self, env):
        ...
        def act(obs, info):  # pylint: disable=unused-argument
            ...
            pick_pix = utils.sample_center_distribution(pick_prob)
            # For "deterministic" demonstrations on insertion-easy, use this:
            # pick_pix = (160,80)
            pick_pos = utils.pix_to_xyz(pick_pix, hmap,
                                        self.bounds, self.pix_size)
            #pick_pose = (np.asarray(pick_pos), np.asarray((0, 0, 0, 1)))
            pick_obj_id = utils.get_nearest_object_id(pick_pos)
            _, obj_ori = p.getBasePositionAndOrientation(pick_obj_id)
            # obj_euler = p.getEulerFromQuaternion(obj_ori)
            # rotated_euler = (obj_euler[0], obj_euler[1], obj_euler[2] - np.pi / 3)
            # rotated_obj_ori = p.getQuaternionFromEuler(rotated_euler)

            original_theta = p.getEulerFromQuaternion(obj_ori) # obj_ori: euler
            object_length_original = utils.get_object_length_along_axis(pick_obj_id, original_theta[2]) # get bbox dim in original
            
            z_theta = original_theta[2] + np.pi / 2
            if z_theta > np.pi: # if bigger than 2pi
                z_theta -= 2 * np.pi
            elif z_theta < -np.pi:
                z_theta += 2 * np.pi
            rotated_theta = (original_theta[0], original_theta[1], z_theta) # bbox dim in 90 degree rotation
            object_length_rotated = utils.get_object_length_along_axis(pick_obj_id, rotated_theta[2])
            # print(rotated_theta) # (0,0,??)
            # print(original_theta) # (0,0,??)
            if object_length_original > object_length_rotated:
                pick_pose = (np.asarray(pick_pos), p.getQuaternionFromEuler([0,0,rotated_theta[2]]))
            else:
                pick_pose = (np.asarray(pick_pos), p.getQuaternionFromEuler([0,0,original_theta[2]]))
            # pick_pose = (np.asarray(pick_pos), np.asarray(obj_ori))
            ...
            return {'pose0': pick_pose, 'pose1': place_pose}, cmap
