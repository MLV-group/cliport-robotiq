...
class PackingBoxes(Task):
    ...
    def reset(self, env):
        ...
        w_inc, h_inc = 0.05, 0.05
        inc_size = np.float32([zone_size[0] + w_inc, 
                                    zone_size[1] + h_inc, 
                                    zone_size[2]]) 
        inc_half = inc_size / 2
        # half = np.float32(zone_size) / 2
        replace = {'DIM': inc_size, 'HALF': inc_half}
        ...
        object_points = {}
        object_ids = []
        max_objects = random.randint(3, 5)
        indices = np.linspace(0, len(bboxes) - 1, max_objects, dtype=int)
        bboxes = np.array(bboxes)
        bboxes = bboxes[indices] # bbox = [min_x, min_y, min_z, max_x, max_y, max_z]
        object_template = 'box/box-template.urdf'
        
        zone_area = zone_size[0] * zone_size[1]
        current_area = 0 # cumulated area
        for i, bbox in enumerate(bboxes):
            size = bbox[3:] - bbox[:3] # box length
            
            box_area = size[0] * size[1]
            
            next_idx = min(i+1, len(bboxes) - 1)
            next_box_size = bboxes[next_idx][3:] - bboxes[next_idx][:3]
            next_box_area = next_box_size[0] * next_box_size[1]
            if current_area + next_box_area > zone_area - next_box_area - 0.0005: # if current area is greater than zone area, break (0.001 is error margin)
                break
            current_area += box_area
            
            position = size / 2. + bbox[:3] # bbox center pt
            position[0] += -zone_size[0] / 2
            position[1] += -zone_size[1] / 2 # position in zone pose
            pose = (position, (0, 0, 0, 1)) # position of zone
        ...
        self.goals.append((
            ...
            (object_points, [(zone_pose, inc_size)]), 1))
