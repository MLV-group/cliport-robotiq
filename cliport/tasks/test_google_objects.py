...
class TestGoogleObjects(Task):
    ...
    def reset(self, env):
        ...
        object_points = {}
        object_ids = []
        # bboxes = np.array(bboxes) # 첫번째 물체만
        bbox = bboxes[0]
        # bboxes = np.array(bboxes)
        print(bbox)
        scale_factor = 5
        object_template = 'google/object-template.urdf'
        chosen_objs, repeat_category = self.choose_objects(object_names, self.object_num)
        print(chosen_objs)
        object_descs = []
        ...
        self.set_goals(object_descs, object_ids, object_points, repeat_category, zone_pose, inc_size)

        for i in range(480):
            p.stepSimulation()

    def choose_objects(self, object_names, object_num):
        repeat_category = None
        return object_names[object_num], repeat_category

    def set_goals(self, object_descs, object_ids, object_points, repeat_category, zone_pose, zone_size):
        object_ids = object_ids[0][0]
        true_poses = []

        true_poses.append(zone_pose)

        chosen_obj_pts = dict()
        chosen_obj_pts[object_ids] = object_points[object_ids]

        self.goals.append(([(object_ids, (0, None))], np.int32([[1]]), [zone_pose],
                            False, True, 'zone',
                            (chosen_obj_pts, [(zone_pose, zone_size)]), # param
                            1))
