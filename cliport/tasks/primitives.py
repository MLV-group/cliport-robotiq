...
class PickPlace():
    ...
    def __call__(self, movej, movep, ee, pose0, pose1):
        ...
        # Activate
        obj_id = None
        while True:  # and target_pose[2] > 0:
            targ_pose = utils.multiply(targ_pose, delta)
            timeout |= movep(targ_pose)
            obj_id = ee.detect_contact()
        ...
        # new_euler = (0, 0, obj_euler[2])
        # new_ori = p.getQuaternionFromEuler(new_euler)
        new_ori = (0,0,0,pick_pose[1][3])
        new_pose = (obj_pos, pick_pose[1])

        timeout |= movep(new_pose, self.speed)
        
        # Activate end effector, move up, and check picking success.
        ee.activate()
        # while True:
        #   if ee.detect_contact() == obj_id:
        #     print("-------")
        #     break
